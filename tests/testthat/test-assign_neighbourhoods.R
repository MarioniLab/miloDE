...
test_that("Wrong input gives errors", {
  ...
  expect_error(assign_neighbourhoods(sce_mouseEmbryo , k = 10, prop = 0.2, order = 3, filtering = T, reducedDim_name = "pca_azimuth", k_init = 50, d = 30),
               "Check order - should be either 1 (standard kNN-graph) or 2 (2nd-order kNN-graph)",
               fixed=TRUE
  )
  expect_error(assign_neighbourhoods(sce_mouseEmbryo , k = 10, prop = 0.2, order = "a", filtering = T, reducedDim_name = "pca_azimuth", k_init = 50, d = 30),
               "Check order - should be either 1 (standard kNN-graph) or 2 (2nd-order kNN-graph)",
               fixed=TRUE
  )
