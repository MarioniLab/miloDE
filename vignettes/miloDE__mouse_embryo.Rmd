...
# packages for gene cluster analaysis
library(scWGCNA)
...
# load chimera Tal1
sce = suppressMessages(Tal1ChimeraData())
...
To assign neighbourhoods, use `assign_neighbourhoods`. Note that under the hood, there is a probabilistic sampling of index cells --> if you want to ensure the same neighbourhood assignment, please set seed prior to running this function.
...
Below we outline a custom script to detect gene modules using WGCNA approach.
...
<!-- get_go_terms = function(genes , module , pval.thresh = 0.1){ -->
...
<!--     p = plotEnrich(gos , numChar = 120) + ggtitle(module) +  -->
...
<!-- plots = lapply(sort(unique(modules_wgcna_all$cluster)) , function(cluster){ -->
<!--   p = get_go_terms(genes = modules_wgcna_all$SYMBOL[modules_wgcna_all$cluster == cluster], module = cluster)  -->
<!--   print(p) -->
<!-- }) -->
