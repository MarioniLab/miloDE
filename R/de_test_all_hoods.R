...
de_test_all_hoods = function(sce ,
  ...
  if (length(genes) == 0){
    ...
    if (gene_selection == "all"){
      ...
      keep <- filterByExpr(y, group=summed$sample.id , min.count = min_count , min.total.count = round(min_count * 1.5))
      if (sum(keep) == 0){
...
#' @importFrom SingleCellExperiment colData SingleCellExperiment counts
#' @importFrom SummarizedExperiment assay
#' @importFrom edgeR DGEList glmQLFit glmQLFTest calcNormFactors filterByExpr estimateDisp
#' @importFrom tibble rownames_to_column
#' @importFrom scuttle summarizeAssayByGroup
...
de_test_single_hood = function(sce , nhoods_sce , hood_id , sample_id , condition_id , covariates = NULL,
  ...
  if (ncol(current.sce) > 0){
    ...
    if (ncol(tab) < 2){
      ...
      stat = .return_null_stat(genes , sufficient_n_samples = FALSE , design_matrix_suitable = FALSE)
    }
    else {
      ...
      stat = tryCatch(
        ...
        error=function(dump){
          warning(paste0("For hood_id " , hood_id , " test can not be performed with given design matrix. Reassess the covariates."))
...
.filter_not_perturbed_hoods <- function(sce){
  ...
  auc_stat = lapply(colnames(nhoods_sce) , function(hood_id){
    out = .get_auc_single_hood(sce , nhoods_sce , hood_id , min_cells = 3)
    return(out)
  })
  auc_stat = do.call(rbind , auc_stat)
  auc_stat$Nhood_id = colnames(nhoods_sce)

  hoods_keep = auc_stat$Nhood_id[auc_stat$auc > 0.5]
...
.get_auc_single_hood = function(sce , nhoods_sce , hood_id , min_cells = 3){
