...
#' @param assay_type A string specifying the assay in SCE containing the expression values to be used (default = "logcounts")
#' @param reduction_type Either "Azimuth" or "MNN" - specifying which method to use for the embedding estimation
#' @param reducedDim_name String specifying the name under which to save the joint embedding
...
#' @param cell_id Character specifying which variable should be used as a cell id (if colnames(sce) are not specified).
...
add_embedding = function(sce ,
  ...
  out = .general_check_arguments(args) & .check_genes_in_sce(sce , genes) & .check_cell_id_in_sce(sce , cell_id) & .check_sample_in_coldata_sce(sce , sample_id)
  ...
  if (!is.null(genes)){
    ...
    dec.sce = modelGeneVar(sce , assay.type = assay_type)
  ...
  if (is.null(colnames)){
    ...
    colnames(sce_w_embedding) = meta[, cell_id]
  }
  ...
  # if assay_type == "counts" and reduction_type == "MNN" -> warn the user
  if (assay_type == "counts" & reduction_type == "MNN"){
  ...
  if (reduction_type == "MNN"){
    sce_w_embedding = .add_mnn_based_embedding(sce_w_embedding , assay_type = assay_type , reducedDim_name = reducedDim_name, sample_id = sample_id, cells_ref = cells_ref, cells_query = cells_query, d = d)
  }
  else if (reduction_type == "Azimuth"){
    sce_w_embedding = .add_azimuth_embedding(sce_w_embedding , reducedDim_name = reducedDim_name, sample_id = sample_id, cells_ref = cells_ref, cells_query = cells_query, d = d)
  }
  sce = sce[ , order(colnames(sce))]
  sce_w_embedding = sce_w_embedding[ , order(colnames(sce_w_embedding)) ]
  reducedDim(sce , reducedDim_name) = reducedDim(sce_w_embedding , reducedDim_name)
  return(sce)
...
.add_mnn_based_embedding = function(sce , assay_type = "logcounts" , reducedDim_name , sample_id = "sample", cells_ref , cells_query , d = 30){
  out = .check_assay_in_sce(sce , assay_type)
  ...
  pca_full = multiBatchPCA(sce_ref , batch = factor(meta_ref[, sample_id]), d = d, preserve.single = TRUE, assay.type = assay_type)
  pca_ref <- pca_full[[1]]
  to_proj <- (as.matrix(assay(sce_query , assay_type)) - rowMeans(as.matrix(assay(sce_query , assay_type))))/sqrt(rowVars(as.matrix(assay(sce_query , assay_type))))
  ...
  # batch correct
  pca_joint = reducedMNN(pca_joint , batch = factor(c(meta_ref[, sample_id], meta_query[, sample_id])) )
  ...
  reducedDim(sce , reducedDim_name) = pca_joint
  return(sce)
}
...
#' @importFrom SingleCellExperiment reducedDim
.add_azimuth_embedding = function(sce , reducedDim_name , sample_id = "sample", cells_ref , cells_query , d = 30){
  sce_ref = sce[ , colnames(sce) %in% cells_ref]
  sce_query = sce[ , colnames(sce) %in% cells_query]
  ...
  samples_ref = unique(meta_ref[ , sample_id])
  samples_query = unique(meta_query[ , sample_id])

  sce_seurat <- CreateSeuratObject(counts = counts(sce))
  sce_seurat = AddMetaData(sce_seurat, as.data.frame(colData(sce)), col.name = NULL)
  sce_seurat.list <- SplitObject(sce_seurat, split.by = sample_id)
  # normalise
