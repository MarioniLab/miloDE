...
#' @param n_threads Positive integer specifying the number of cores to be used to calculate AUC. Higher number results in faster calculation, but its feasibility depends on the specs of your machine.
#'
#' @return
#' @export
...
#'
calc_AUC_per_neighbourhood <- function(sce , sample_id = "sample" , condition_id, min_n_cells_per_sample = 1, n_threads = 2){
  ...
  auc_stat = auc_stat[, c("Nhood", "Nhood_id" , "auc")]
  ...
  if (ncol(current.sce) > 0){
    ...
    if (length(tab) == 2 & tab[1] >= min_cells & tab[2] >= min_cells){
      auc = calculate_auc(logcounts(current.sce), meta, cell_type_col = "celltype.dummy",
                          ...
                          feature_perc = 1 , n_threads = n_threads)
